...
case class Ping(override val sourceHexapod: UUID,
  ...
  extends Message("ping", true, sourceHexapod, destinationHexapod, transportEndpoint) {
  ...
  def react(stimulus: Stimulus) = stimulus match {
    case Stimulus.IncomingMessage(message @ Ping(_, _, _)) if message.conversation.compareTo(conversation) == 0 =>
      Some(true)
    case _ =>
      None
  }
  override def toString = "Ping[%08X]".format(this.hashCode())
}
